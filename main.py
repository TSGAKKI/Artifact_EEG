...
def main(args):
    ...
    for i in range(k):
        ...
        print('EEG_val:',np.shape(EEG_val)) 
            ...
            #--input data
        train_x = torch.from_numpy(partial_noiseEEG_train).type(torch.FloatTensor)#.to(DEVICE)  # (B, N, F, T)
        train_target = torch.from_numpy(partial_EEG_train).type(torch.FloatTensor)#.to(DEVICE)  # (B, N, T)
        ...
        if args.do_train:
            ...
            # Train
            train(model, train_loader,val_loader, args, device, args.save_dir, log)#, tbx
...
def train(model, train_loader, val_loader,args, device, save_dir, log): #, tbx
    ...
    while (epoch != args.num_epochs) and (not early_stop):
        ...
        if epoch % args.eval_every == 0:
            ...
            if eval_results < prev_val_loss:
                patience_count = 0
            else:
                patience_count += 1
            prev_val_loss = eval_results
